...
class PositionalEncoding(nn.Module):
    def __init__(self , d_model: int , seq_len:int , dropout : float ) :
        ...
        position = torch.arange(0 , seq_len -1 , dtype = torch.float).unsqueeze() # (seq_len , 1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float()*(-math.log(10000.0)/d_model))
        #div_term = torch.pow(10000 , torch.arange(0 , d_model , 2).float()/d_model)
        #apply sine to even and cos to odd positions
...
class MultiHeadAttentionBlock(nn.module):
    def __init__(self, d_model:int , h:int , dropout = float):
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_model is not divisible by num heads"
...
class Transformer(nn.Module):
    ...
    def __init__(self , encoder: Encoder , decoder: Decoder , src_embed:InputEmbeddings , tgt_embed : InputEmbeddings,
                 src_pos:PositionalEncoding,tgt_pos: PositionalEncoding , projecttion_layer : ProjectionLayer) -> None:
        super().__init__()
        ...
        self.projection_layer = projecttion_layer
